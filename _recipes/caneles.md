---
layout: recipe
title: Canelés
source: "Maman: The Cookbook, Elisa Marshall & Benjamin Sormonte with Lauren Salkeld (Clarkson Potter, 2021)"
date: 2026-07-08
servings: 12
ingredients:
  - name: whole milk
    amount: 480
    unit: mL
  - name: sugar
    amount: 272
    unit: g
  - name: unsalted butter
    amount: 15
    unit: mL
  - name: vanilla bean, split lengthwise
    amount: 1
    unit: count
  - name: large egg yolks
    amount: 3
    unit: count
  - name: dark rum (such as Plantation Original Dark Rum)
    amount: 30
    unit: mL
  - name: fine sea salt
    amount: 1.25
    unit: mL
  - name: all-purpose flour
    amount: 145
    unit: g
  - name: vegetable oil spray, for mold
    amount: 1
    unit: count
---

_Makes 12 canelés._

### Make the batter

1. In a small saucepan, combine milk, ¾ cup (136 g) of sugar, and butter. Scrape in vanilla seeds and add pod.
2. Bring to a boil over medium-high heat, keeping a close eye.
3. Remove from heat, add flour, and stir vigorously with a wooden spoon until fully incorporated.
4. Return to very low heat and cook, stirring, about 1 minute, until dough is drier and no longer sticks to pan.
5. Meanwhile, in a stand mixer fitted with whisk attachment, whip egg yolks, ⅔ cup (136 g) of sugar, rum, and salt on medium until pale yellow, about 2 minutes.
6. Discard vanilla pod from milk mixture. With mixer on low, slowly add hot milk and whip until fully combined, 2 to 3 minutes.
7. Gradually add flour in 4 batches, whipping until fully combined after each addition, about 4 minutes total.
8. Pour batter through a sieve into a 960 mL spouted measuring cup.

### Chill

1. Cover with plastic wrap and refrigerate for at least 24 hours and up to 48 hours.

### Bake

1. Set a rack in center of oven and preheat to 450°F (230°C). Coat a canelé mold with vegetable oil spray, using a pastry brush to cover all crevices.
2. Remove batter from refrigerator, give it a stir, and fill each mold about three-quarters full.
3. Bake for 10 minutes, then reduce oven to 375°F (190°C), rotate as needed, and continue baking until tops are golden brown, 30 to 35 minutes.
4. With a small knife, remove each canelé from pan and set on a wire rack to cool for about 30 minutes.
